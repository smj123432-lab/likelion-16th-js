...
async function handleLoginSubmit(event) {
  ...
  try {
    ...
    const results = await Promise.allSettled([
      fetch(`${API_BASE_URL}/carts/user/${user.id}`).then((response) =>
        response.json(),
      ),
      fetch(`${API_BASE_URL}/products?limit=2`).then((response) =>
        response.json(),
      ),
      fetch(`${API_BASE_URL}/posts/user/${user.id}`).then((response) =>
        response.json(),
      ),
      fetch(`${API_BASE_URL}/todos/user/${user.id}`).then((response) =>
        response.json(),
      ),
    ]);
...
function renderDashboard(user, data) {
  ...
  contentDisplay.innerHTML = `
    ...
    ${createDataCard("🛒 장바구니", carts, (val) =>
      val.carts.length
        ? `ID: ${val.carts[0].id} 포함 ${val.carts.length}건`
        : "비어있음",
    )}
