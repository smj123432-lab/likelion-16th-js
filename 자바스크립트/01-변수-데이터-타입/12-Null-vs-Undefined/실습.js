...
function greet(name, age) {
  return name + '는 ' + age + '살이다.'
}
console.log(greet('사민재', 26));
...
let currentUser = null // 로그인 x (시간이 지남에 따라 변하는 데이터)
...
user.ProfileImage = 'http://images.pexels.com'
  user.bio = '멋사 프론트엔드 16기 수강생'
  console.log(user);
}
// user 객체 출력
...

{
  ...
  // 표현식에서의 논리곱(&&, AND) 연산자
  // - 첫 번째 거짓 같은 값(Falsy)를 찾는 연산자 false를 찾는다.
  let result1 = 1 && {} && '' && -1 && 10n && [0, 1, 2]
  console.log(result1)

  // 표현식에서의 논리합(||, OR) 연산자
  // - 첫 번째 참 같은 값(Truthy)를 찾는 연산자 true를 찾는다.
