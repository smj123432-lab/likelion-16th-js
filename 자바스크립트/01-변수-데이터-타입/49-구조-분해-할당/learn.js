...
// 객체의 속성 이름을 범위 내 변수로 정의하고
// 변수에 각 속성에 매칭되는 값을 할당한다면?
// console.log(myNotebook);
{
  const name = myNotebook.name;
  const cpu = myNotebook.cpu;
  const ram = myNotebook.ram;
  const ssd = myNotebook.ssd;
  const maker = myNotebook.maker;

  console.log(name, cpu, ram, ssd, maker);
}

{
  const { name: brand, cpu: processor, ram, ssd, maker } = myNotebook;
...
console.log(rem2px("3.145px", "10px")); // '31.45px'em'
console.groupEnd();
