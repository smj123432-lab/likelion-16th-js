...
// 복잡한 데이터 필터링
{
  ...
  foundPeople = users.filter((user) =>
    user.email.includes(emailService.toLowerCase()),
  );

  // 구조 분해 할당 문법을 사용해 user 객체의 email 속성을 매개변수로 빼낸 경우
  foundPeople = users.filter(({ email }) =>
    email.includes(emailService.toLowerCase()),
  );
...
const buttonData = [
  ...
  },
  {
    ...
    type: "submit",
    label: "로그인",
    message: "사용자 계정으로 로그인되었습니다.",
  },
  {
    id: "button-login",
    type: "reset",
    label: "초기화",
    message: "입력 내용을 모두 초기화했습니다.",
  },
  {
    id: "button-readmore",
    type: "button",
    label: "더보기",
    message: "감춰진 데이터를 더보기합니다.",
  },
];
